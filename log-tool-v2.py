...
def limit_loacsearch_trace(text):
    """Determine location search row that should be ignored."""
    if args.show_all:
        return False
    else:
        for index in range(len(loc_ignore_list)):
            if text.find(loc_ignore_list[index].ident_text) != -1:
                return True
    return False
...
#def parse_file( debug_file, output_name, output_dir):
#    """
...
log_file    = open(file=args.log_file_name,encoding='iso-8859-1')
...
try:
    ...
    for ignore_line in loc_ignore_file:
        ...
        loc_ignore_list.append(add_row)
...
for line in log_file:
    ...
    if matchObj:
        ...
        if is_loc_search_trace(line):
            if not limit_loacsearch_trace(line):
