...
# Trace config class
class TraceConf:
   'Common base class for Trace color config data'
   tot_row = 0
...
def get_row_type(text):
    ...
    for index in range(len(trace_conf_list)):
        if text.find(trace_conf_list[index].ident_text) != -1:
            return index;
    return -1;

def get_row_color(id):
    """Get row color """
    if id == -1:
        return '"black"'
    return '"' + trace_conf_list[id].color_text + '"'
...
def do_db_error_table(out_file):
    """Write a table with all db errors."""

    db_line = "<table style=%s>\n<tr>" % ('"width:150px"')
    out_file.write(db_line)
    out_file.write("<td>DB  -ERROR</td>\n</tr>")

    for index in range(len(db_list)):
        db_line = "<td><a href=%s>%s</a></td>\n</tr>" % ('"#' + str(db_list[index].row_id) + '"', db_list[index].row_id)
        out_file.write(db_line)
...
def do_appl_error_table(out_file):
    """Write a table with all APPL-ERROR."""

    appl_line = "<table style=%s>\n<tr>" % ('"width:150px"')
    out_file.write(appl_line)
    out_file.write("<td>APPL-ERROR</td>\n</tr>")

    for index in range(len(appl_list)):
        appl_line = "<td><a href=%s>%s</a></td>\n</tr>" % ('"#' + str(appl_list[index].row_id) + '"', appl_list[index].row_id)
        out_file.write(appl_line)
    out_file.write("</table>\n")
...
def do_call_trace_table(out_file):
    """Write a table with all Calltrace."""

    trace_line = "<table style=%s>\n<tr>" % ('"width:150px"')
    out_file.write(trace_line)
    out_file.write("<td>CallTrace</td>\n</tr>")

    for index in range(len(trace_list)):
        trace_line = "<td><a href=%s>%s</a></td>\n</tr>" % ('"#' + str(trace_list[index].row_id) + '"', trace_list[index].row_id)
        out_file.write(trace_line)
    out_file.write("</table>\n")

def do_syspar_trace_table(out_file):
    """Write a table with all System Parameter errors."""

    trace_line = "<table style=%s>\n<tr>" % ('"width:150px"')
    out_file.write(trace_line)
    out_file.write("<td>SysPar missing</td>\n</tr>")

    for index in range(len(syspar_list)):
        syspar_line = "<td><a href=%s>%s</a> %s </td>\n</tr>" % ('"#' + str(syspar_list[index].row_id) + '"', syspar_list[index].row_id, syspar_list[index].syspar)
...
def do_html_file_reference(out_file, file_text, to_file):
    """Write html link to other files."""

    print to_file
    # print_h_line = "<hr>\n<h3>%s <a href=%s>%s</a></h3>\n</hr>\n" % (file_text,  '"' + to_file + '"', to_file)
    print_h_line = "<hr>\n<h3> <a href=%s>%s</a></h3>\n</hr>\n" % ('"' + to_file + '"', file_text)
    out_file.write(print_h_line)
...
def do_html_end_wrapped_mark(out_file):
    ...
    print_h_line = "<p style=%s >" % ('"background-color:#F8F8F8"')
    out_file.write(print_h_line)
    if wrapped_line < 0:
...
try:
    trace_config_file    = open(config_path + 'trace-color.conf')
    for conf_line in trace_config_file:
        tmp_conf = conf_line.rstrip()
        parts = tmp_conf.split(';',2)
        add_row = TraceConf(parts[0],parts[1])
        trace_conf_list.append(add_row)
...
if len(gwf_list) > 0:
    ...
    do_html_file_reference(gwf_output_file, "Main trace ", trace_file_name)
...
if len(loc_search_list) > 0:
    ...
    do_html_file_reference(loc_search_output_file, "Main trace ", trace_file_name)
...
if len(box_calc_list) > 0:
    ...
    do_html_file_reference(box_calc_output_file, "Main trace ", trace_file_name)
...
do_html_file_reference(output_file, "GWF trace ", gwf_trace_file_name)
do_html_file_reference(output_file, "Location Search ", loc_search_trace_file_name)
do_html_file_reference(output_file, "Box Calc ", box_calc_trace_file_name)
table_line = "<table style=%s>\n<tr>" % ('"width:300px"')
...
for index in range(len(line_list)):
    ...
    print_line = "<FONT COLOR=%s>%8.1d <a name=%s></a> <FONT COLOR=%s>%s <FONT COLOR=%s>%s" % ('"grey"',index, '"' + str(index) + '"',  '"green"',line_list[index].time_stamp, get_row_color(line_list[index].row_type),line_list[index].row_text)
